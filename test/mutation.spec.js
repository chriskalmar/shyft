...
describe('mutation', () => {
  it('reject mutations without state transitions on stateful entities', async () => {
    ...
    await mutate(BoardMember, 'create', payload, null, asUser(99)).catch(e => {
  ...
  it('fail mutation if fromState is not a match', async () => {
    ...
    await mutate(BoardMember, 'accept', {}, inviteId, asUser(81)).catch(e => {
      expect(e).toMatchSnapshot();
    });
  });
  ...
  it('reject unknown mutations', async () => {
    await mutate(BoardMember, 'findInnerPeace', {}, 1, asUser(80)).catch(e => {
      expect(e).toMatchSnapshot();
    });
  });
});
