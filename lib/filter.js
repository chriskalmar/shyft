...
const defaultTransformFilterAttributeName = (name) => name

export const transformFilterAttributeName = (context) => i18nTransformFilterAttributeName(context)


const buildWhereAttributeOperatorConditionQuery = (qBuilder, attributeName, operator, value, transformFilterAttributeNameFn = defaultTransformFilterAttributeName, _placeholderIdx = null) => {
  ...
  switch (operator) {
    ...
    case '$in':
      ...
      break;
    case '$notIn':
      qBuilder.andWhere(`${leftExpression} NOT IN (:${placeholderName})`, data)
      break;
    ...
    case '$contains':
      qBuilder.andWhere(`${leftExpression} ILIKE :${placeholderName}`, { [placeholderName]: `%${escapeILikePattern(value)}%` })
      break;
    case '$startsWith':
      qBuilder.andWhere(`${leftExpression} ILIKE :${placeholderName}`, { [placeholderName]: `${escapeILikePattern(value)}%` })
      break;
    ...
    case '$notContains':
      qBuilder.andWhere(`${leftExpression} NOT ILIKE :${placeholderName}`, { [placeholderName]: `%${escapeILikePattern(value)}%` })
      break;
    case '$notStartsWith':
      qBuilder.andWhere(`${leftExpression} NOT ILIKE :${placeholderName}`, { [placeholderName]: `${escapeILikePattern(value)}%` })
      break;
...
const buildWhereAttributeConditionQuery = (qBuilder, attributeName, filter, transformFilterAttributeNameFn, _placeholderIdx) => {
  const placeholderIdx = _placeholderIdx || { idx: 0 }
  placeholderIdx.idx++
  ...
  if (isMap(filter)) {
    ...
    operators.map(operator => {
      ...
      buildWhereAttributeOperatorConditionQuery(qBuilder, attributeName, operator, value, transformFilterAttributeNameFn, placeholderIdx)
    })
  }
  else {
    buildWhereAttributeOperatorConditionQuery(qBuilder, attributeName, '$eq', filter, transformFilterAttributeNameFn, placeholderIdx)
  }
}



const buildWhereTypeSubQuery = (qBuilder, filter, entityName, modelRegistry, isGetMany, transformFilterAttributeNameFn, _placeholderIdx) => {
  if (!filter.entity) {
  ...
  const { dataShaperMap: targetDataShaperMap, storageTableName: filterEntityStorageTableName } = modelRegistry[filter.entity]
  ...
  qBuilder.andWhere(qbSub => {
    ...
    conditions.map(({ targetAttribute, operator, sourceAttribute, value }) => {
      ...
      if (sourceAttribute) {
        ...
        buildWhereAttributeOperatorConditionQuery(subQuery, targetAttributeName, operator, value, transformFilterAttributeNameFn, _placeholderIdx)
      }
    })
...
export const buildWhereTypeQuery = (filter, entityName, modelRegistry, isGetMany, transformFilterAttributeNameFn, _placeholderIdx) => {
  const placeholderIdx = _placeholderIdx || { idx: 0 }
  placeholderIdx.idx++
        ...
        keys.map(key => {
          ...
          if (key === '$and') {
            ...
            if (andFilters.length) {
              newFilter.map(val => {
                const where = buildWhereTypeQuery(val, entityName, modelRegistry, isGetMany, transformFilterAttributeNameFn, placeholderIdx)
                if (where) {
            ...
            buildWhereTypeSubQuery(qbAnd, newFilter, entityName, modelRegistry, isGetMany, transformFilterAttributeNameFn, _placeholderIdx)
          }
          else if (key === '$not') {
            ...
            if (isMap(newFilter, true)) {
              const where = buildWhereTypeQuery(newFilter, entityName, modelRegistry, isGetMany, transformFilterAttributeNameFn, placeholderIdx)
              if (where) {
            ...
            buildWhereAttributeConditionQuery(qbAnd, attributeName, newFilter, transformFilterAttributeNameFn, placeholderIdx)
          }
        })
...
export const buildWhereQuery = (qBuilder, filter, entityName, modelRegistry, isGetMany, context) => {

  if (!modelRegistry || !modelRegistry[entityName]) {
    throw new Error('buildWhereQuery() is missing a valid modelRegistry parameter')
  }

  const transformFilterAttributeNameFn = transformFilterAttributeName(context)

  const pureFilter = purifyFilter(filter)
  const where = buildWhereTypeQuery(pureFilter, entityName, modelRegistry, isGetMany, transformFilterAttributeNameFn)
