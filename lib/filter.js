...
export const buildWhereQuery = (qBuilder, filter, entityName, _placeholderIdx) => {
  const placeholderIdx = _placeholderIdx || { idx: 0 }
  placeholderIdx.idx++
        ...
        keys.map(key => {
          ...
          if (key === '$and') {
            ...
            if (andFilters.length) {
              newFilter.map(val => {
                const where = buildWhereQuery(qbAnd, val, entityName, placeholderIdx)
                if (where) {
            ...
            if (orFilters.length) {
              const orBrackets = new Brackets(qbOr => {
                newFilter.map(val => {
                  const where = buildWhereQuery(qbOr, val, entityName, placeholderIdx)
                  if (where) {
                    qbOr.orWhere(where)
                  }
                })
