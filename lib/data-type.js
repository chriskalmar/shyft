...
import { schemaDefinitions } from './validation/base';
...
class DataType {
  ...
  constructor (name, jsonSchemaType, dbType) {
    ...
    // auto generate a definition validation function
    this.buildDefinitionValidationFunction()
  }
  ...
  // build a validation function for the data type definition
  buildDefinitionValidationFunction () {
    this.definitionValidator = validator.validatorFromJsonSchema( this.generateDefinitionJsonSchema() )
  }


  // get the definition validation function
  getDefinitionValidator () {

    // if not define yet build it now
