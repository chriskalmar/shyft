...
function bootstrap() {
  ...
  registry.setCoreComponent( initComponent('schema') )
  ...
  function initComponent(name) {
    const klass = require(`./components/${name}`).default
    return new klass()
  }
}
...
function generateDatabaseSql(models) {
  ...
  const mainTemplate = loadTemplate(`${__dirname}/components/main.tpl.js`);
  ...
  // render
  return mainTemplate(templateVars)
}
