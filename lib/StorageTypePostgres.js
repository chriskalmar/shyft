...
const processSequelizeError = (err, sequelize) => {
  ...
  if (err instanceof sequelize.UniqueConstraintError) {
    throw new CustomError('Unique constraint error', 'UniqueConstraintError')
  }
...
export const StorageTypePostgres = new StorageType({
  ...
  async mutate (entity, id, source, args, dataKey, entityMutation, context, info, relayTypePromoterField) {
    ...
    if (entityMutation.type === MUTATION_TYPE_CREATE || entityMutation.type === MUTATION_TYPE_UPDATE) {
      ...
      mapOverProperties(attributes, ({isSystemAttribute, defaultValue}, attributeName) => {
        if (isSystemAttribute && defaultValue) {
          const value = defaultValue(data, entityMutation)
