    ...
    _.map(entity.getAttributes(), (attribute) => {
      ...
      if (isDataTypeEnum(attribute.type)) {
        ...
        if (attribute.isPrimary) {
          ...
          PrimaryGeneratedColumn(attributes[attributeName])(Skeleton.prototype, attributeName)
        }
        else {
...
export const generateMockData = async (storageInstance, schema, languages=['en']) => {
  ...
  for (let e=0; e < dependencySortedEntityNames.length; e++) {
    ...
    try {
      for(let i=0; i < _.random(10, 100); i++) {
        await generateItem(storageInstance, entity, mockInstancesMemory, languages)
      }
    }
    catch(err) {
      console.error(err)
...
async function generateItem(storageInstance, entity, mockInstancesMemory, languages) {
  ...
  _.map(entity.getAttributes(), ({ type, isPrimary, required, mock }, name) => {
    ...
    if (isComplexDataType(type)) {
      ...
      if (mockDataGenerator) {
        item[name] = mockDataGenerator(entity, name, languages)
      }
    }
  })
