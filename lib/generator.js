...
export const installStorageScripts = async (configuration, synchronize=false) => {
  ...
  await manager.query(storageConfiguration.generateGetLanguageIsoCodeFunction(configuration));
  await manager.query(storageConfiguration.generateGetLanguageIdFunction(configuration));
