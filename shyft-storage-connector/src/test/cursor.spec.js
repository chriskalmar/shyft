...
describe('cursor', () => {
  it('after', async () => {
    const cursor = {
      after: {
        Profile: [ [ 'username', 'amalia943' ], [ 'id', 65 ] ],
      },
    };
  ...
  it('after + filter', async () => {
    const cursor = {
      after: {
        Profile: [ [ 'username', 'ella377' ], [ 'id', 103 ] ],
      },
    };
  ...
  it('before + filter', async () => {
    const cursor = {
      before: {
        Profile: [ [ 'username', 'emilia189' ], [ 'id', 90 ] ],
      },
    };
  ...
  it('after + before + filter + offset', async () => {
    const cursor = {
      after: {
        Profile: [ [ 'username', 'estell684' ], [ 'id', 70 ] ],
      },
      before: {
        Profile: [ [ 'username', 'edward969' ], [ 'id', 8 ] ],
      },
    };
  ...
  it('mutli-key after + before', async () => {
    ...
    const cursor = {
      after: {
        Profile: [ [ 'firstname', 'Joe' ], [ 'lastname', 'Ratke' ], [ 'id', 104 ] ],
      },
      before: {
        Profile: [ [ 'firstname', 'Jose' ], [ 'lastname', 'Leffler' ], [ 'id', 71 ] ],
      },
    };
  ...
  it('time-based after + first', async () => {
    ...
    await Promise.all(
      differentiatorId.map(async id => {
        const cursor = {
          after: {
            Message: [ [ 'writtenAt', '2018-03-26 20:47:46.578Z' ], [ 'id', id ] ],
          },
        };
